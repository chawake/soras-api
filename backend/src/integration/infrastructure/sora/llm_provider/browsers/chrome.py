...
class ChromeBrowser(BaseBrowser):
    def __init__(self, rest_adapter: SoraRestAdapter, proxy: Optional[str] = None, browser_name: str | None = None):
        ...
        self.user_agents = UserAgent(
            browsers="Chrome", platforms="desktop", min_version=120.0
        )
        ...
        options.add_argument(
            f"--window-size={random_window_size[0]},{random_window_size[1]}"
        )
        ...
        if self.use_undetected:
            ...
            self.driver.add_cdp_listener('Network.responseReceived', self._cdp_callback)
        else:
    ...
    def _cdp_callback(self, message):
        ...
        try:
            self.rest_adapter.cdp_network_response_received_callback(message, response_body)
